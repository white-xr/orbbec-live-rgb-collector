...
The launcher starts `collect_orbbec_rgb_dataset.py`, which saves RGB images only.
...
```powershell
python collect_orbbec_rgb_dataset.py --camera 335L --task coarse
...
- Local GUI settings are stored in `orbbec_rgb_dataset_gui_settings.json` and are not committed.
